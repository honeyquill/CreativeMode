...
namespace CreativeMode.SpecialBlocks
{
    ...
    {
        //Helpers:
        public static Quaternion BlockToQuanternium(BlockData block)
        {
            Regex regex = new Regex(@"'facing': (.*?)[},]");
            var match = regex.Match(block.properties);
            string Dir = "";
        ...
        public static Vector3 BlockDataToVector3(BlockData block)
        {
            return new Vector3(block.position[0], block.position[1], block.position[2]);
        }

        //Actual blocks
        public static void ChangeSpawn(BlockData block, TeamType team)
        {
            Quaternion LookDir = BlockToQuanternium(block);
       ...
       public static void OffsetBunny()
       {
        ...
        public static void SpawnGoal(BlockData Block, TeamType Team)
        {
            Vector3 pos = BlockDataToVector3(Block);
